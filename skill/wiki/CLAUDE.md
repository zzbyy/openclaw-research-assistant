...
These are available when the user prefers explicit commands:

| Command | Script does | You do |
|---------|------------|--------|
| `/wiki batch` | Extract → dedup → list entries | Absorb entries (with subagents if many) |
...
| `/wiki catalog` | Scan sources | Present summary |
| `/wiki reindex` | Update QMD index | Report result |
